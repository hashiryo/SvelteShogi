...
KifuNode->>Domain : sfenxToShogiPosition() / strToPosition()
...
2. `sfenxToShogiPosition(node.sfenx)` でSFENX文字列を盤面データに変換。
...
- **問題**: `jumpToKifu` を呼び出したが、局面が更新されない。
  - **原因**: `setCurrentNode` 内で `sfenxToShogiPosition` がエラーを起こしている可能性。SFENX文字列が不正であるか、`strToPosition` の入力が誤っている。
