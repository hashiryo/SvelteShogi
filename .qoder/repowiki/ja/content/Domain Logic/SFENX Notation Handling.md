...
`sfenxToShogiBoard`関数は、SFENX文字列を内部ゲーム状態オブジェクトに解析します。このオブジェクトには以下が含まれます：
...
`shogiBoardToSfenx`関数は、内部ゲーム状態オブジェクトをSFENX文字列に変換します：
...
PlayShogi->>Sfenx : shogiBoardToSfenx()
...
KifuHistory->>Sfenx : sfenxToShogiBoard(sfenx)
...
- [sfenx.test.ts](file://src/test/domain/sfenx.test.ts#L1-L96) - _追加されたテストケース_
...
- [play-shogi.ts](file://src/handler/play-shogi.ts#L77-L126)
- [sfenx.ts](file://src/domain/sfenx.ts#L181-L238)
- [sfenx.test.ts](file://src/test/domain/sfenx.test.ts#L3-L96) - _テストケースの例_
