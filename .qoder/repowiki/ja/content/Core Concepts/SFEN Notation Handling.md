...
## SFENXパース機構
`sfenxToShogiPosition`関数は、SFENX文字列をアプリケーションのストアと互換性のある内部ゲームステート表現に変換する責任を負います。

### 主要な関数とデータフロー
- **`sfenxToShogiPosition(sfenx: string)`**：入力文字列を分割し、パースを委任するメインエントリポイント。
...
participant Parser as "sfenxToShogiPosition"
...
## SFENXシリアライズ処理
`shogiPositionToSfenx`関数は、現在のゲームステートを保存または共有のためにSFENX文字列にシリアライズします。

### 主な関数
- **`shogiPositionToSfenx(grid, capturedSente, capturedGote)`**：シリアライズプロセスを調整。
...
1. `sfenxToShogiPosition`が文字列をパース
...
```mermaid
graph TB
A[SFENX文字列] --> B[sfenxToShogiPosition]
B --> C{パースされたステート}
C --> D[setGrid]
C --> E[setCaptured]
C --> F[setIsSenteTurn]
...
Controller->>SFENX : sfenxToShogiPosition(node.sfenx)
